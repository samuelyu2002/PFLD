...
class ImageDate():
    def __init__(self, line, imgDir, image_size=112):
        ...
        #140: 图片名称
        # print(len(line))
        if(len(line)!=141): print(line)
        assert(len(line) == 141)
        self.list = line
        self.landmark = np.asarray(list(map(float, line[:136])), dtype=np.float32).reshape(-1, 2)
        self.box = np.asarray(list(map(int, map(float, line[136:140]))),dtype=np.int32)
...
#        self.blur = flag[5]
        self.path = os.path.join(imgDir, line[140])
        self.img = None
...
def get_dataset_list(imgDir, outDir, landmarkDir, is_train):
    with open(landmarkDir,'r') as f:
        ...
        for i, line in enumerate(lines):
            ...
            img_name = Img.path
            Img.load_data(is_train, 16, Mirror_file)
...
if __name__ == '__main__':
    ...
    imageDirs = '300W'
    Mirror_file = '300W/Mirror68.txt'

    landmarkDirs = [
'300W/challenging_anno.txt', '300W/train_anno.txt','300W/test_anno.txt'
                    ]

    outDirs = ['300W/challenging_data', '300W/train_data', '300W/test_data']
    isTrain = [False, True, False]
    for landmarkDir, outDir, is_train in zip(landmarkDirs, outDirs, isTrain):
